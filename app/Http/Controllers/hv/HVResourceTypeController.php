...
use App\Models\hv\HVResourceType;
use App\Utils\Utils;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;

class HVResourceTypeController extends Controller
{
    public function all(Request $request)
    {
        $query = HVResourceType::select()->where('hv_resource_type.hv_company_id', $request->current_company_id);
    ...
    public function createOrUpdate(Request $request, $id = null)
    {
        ...
        if ($id) {
            $obj = HVResourceType::find($id);
            $obj->updated_by = $request->current_user_id;
        } else {
            $obj = new HVResourceType();
        ...
        if (Utils::alreadyExists(HVResourceType::class, $request->current_company_id,'ident', $obj->ident, $obj->id)) {
