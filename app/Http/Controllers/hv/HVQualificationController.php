...
use App\Models\hv\HVQualification;
use App\Utils\Utils;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;

class HVQualificationController extends Controller
{
    public function all(Request $request)
    {
        $query = HVQualification::select()->where('hv_qualification.hv_company_id', $request->current_company_id);
    ...
    public function createOrUpdate(Request $request, $id = null)
    {
        ...
        if ($id) {
            $obj = HVQualification::find($id);
            $obj->updated_by = $request->current_user_id;
        } else {
            $obj = new HVQualification();
        ...
        if (Utils::alreadyExists(HVQualification::class,$request->current_company_id,'ident', $obj->ident, $obj->id)) {
