...
class TMSystemBinaryController extends Controller
{
...
//            $obj->save();
    ...
    {
        $query = TMSystemBinary::select(
            'tm_system_binary.*',
            'tm_system_binary.id as ext_id',
            'hv_user.ident as user_ident'
        )
            ->leftJoin('hv_user', 'hv_user.id', '=', 'tm_system_binary.created_by');
    ...
    public function createOrUpdate(Request $request, $id = null)
    {
        ...
        $all = $request->all();
        if ($id) {
            $obj = TMSystemBinary::find($id);
            $obj->updated_by = $request->current_user_id;
        } else
            if (Utils::varFromArray($all, 'ext_id')) {
                $obj = TMSystemBinary::find($all['ext_id']);
                $obj->updated_by = $request->current_user_id;
                $isNew = true;
            } else {
                $obj = new TMSystemBinary();
        ...
        foreach ($all as $property => $value) {
            switch ($property) {
                ...
                case 'term':
                    if (!$isNew)
                        $this->addToHistory($obj->id, $obj->tm_system_id, $request->current_company_id, $request->current_user_id, 'Datei geändert', $obj->property, $value);
        ...
        if ($isNew)
            $this->addToHistory($obj->id, $obj->tm_system_id, $request->current_company_id, $request->current_user_id, 'Datei hinzugefügt', null, $obj->filename_org);
    ...
    public function delete(Request $request, $id)
    {
        ...
        if ($obj) {
            ...
            $this->addToHistory($obj->id, $obj->tm_system_id, $request->current_company_id, $request->current_user_id, 'Datei ' . $obj->filename_org . ' gelöscht', null, '');
    ...
    public function download(Request $request, $id)
    {
        ...
        if ($obj) {
            ...
            $systemObj = TMSystem::where('id', $obj->tm_system_id)->first();
            if ($systemObj)
