...
class TMCCTestItemController extends Controller
{
    public function all(Request $request)
    {
        $query = TMCCTestItem::select()->where('tm_cc_test_item.hv_company_id', $request->current_company_id);
    ...
    public function createOrUpdate(Request $request, $id = null)
    {
        ...
        if ($id) {
            $obj = TMCCTestItem::find($id);
            $obj->updated_by = $request->current_user_id;
        } else {
            $obj = new TMCCTestItem();
        ...
        if (Utils::alreadyExists(TMCCTestItem::class, $request->current_company_id,'ident', $obj->ident, $obj->id)) {
