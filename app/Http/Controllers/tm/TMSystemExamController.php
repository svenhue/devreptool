...
use App\Models\tm\TMSystemExam;
use App\Utils\Utils;
use Exception;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;

class TMSystemExamController extends Controller
{
...
//            $obj->save();
    ...
    {
        $query = TMSystemExam::select(
            'tm_system_exam.*',
            'tm_exam.ident AS exam_ident'
        )
            ->leftJoin('tm_exam','tm_exam.id','=','tm_system_exam.tm_exam_id')
            ->where('tm_system_exam.hv_company_id', $request->current_company_id);
    ...
    public function createOrUpdate(Request $request, $id = null)
    {
        ...
        if ($id) {
            $obj = TMSystemExam::find($id);
            $obj->updated_by = $request->current_user_id;
        } else {
            $obj = new TMSystemExam();
