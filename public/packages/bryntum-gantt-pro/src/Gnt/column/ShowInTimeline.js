...
Ext.define("Gnt.column.ShowInTimeline", {
    ...
    initComponent : function (config) {
        ...
        this.on('added', this.onShowInTimelineAdded, this);
    },

    onShowInTimelineAdded : function () {
        var grid = this.up('[taskStore]') || this.up('[store]');

        if (grid) {
            if (grid.rendered) {
                this.setDisabled(grid.isReadOnly());
            }

            this.mon(grid, 'setreadonly', this.onGridReadOnlyChange, this);
        }
    },

    onGridReadOnlyChange : function (gantt, readOnly) {
