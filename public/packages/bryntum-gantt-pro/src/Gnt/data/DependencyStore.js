...
Ext.define('Gnt.data.DependencyStore', {
    ...
    normalizeTaskGroups : function(sourceGroup, targetGroup) {
        var i = sourceGroup.length - 1,
            j = targetGroup.length - 1,
            sourceTopParent, targetTopParent;

        // Iterate tasks parents starting from the top until we find mismatching ones 
        while ((sourceTopParent = sourceGroup[i]) && ((targetTopParent = targetGroup[j])) && sourceTopParent == targetTopParent) {
            i--;
            j--;
        }

        // return arrays of parents left
        return [sourceGroup.slice(0, i + 1), targetGroup.slice(0, j + 1)];
    },
    ...
    groupsHasTransitiveDependency : function (sourceId, targetId, ignoreDepRecords, addDepRecords, context) {
        ...
        var groups          = this.normalizeTaskGroups(source.getTopParent(true), ctx.targetGroup),
            sourceGroup     = groups[0],
            targetGroup     = groups[1],
            sourceTopParent = sourceGroup[sourceGroup.length - 1],
            targetTopParent = targetGroup[targetGroup.length - 1];
        ...
        // calculate cache key for provided arguments
        var cacheKey        = (sourceTopParent ? sourceTopParent.getId() : '') + '-' + (targetTopParent ? targetTopParent.getId() : '') + '-' + ignoreDepKey + '-' + addDepKey;
        ...
        // empty top parent on one of sides mean that source and target are child & parent
        if (!sourceTopParent || !targetTopParent) {
            return this.setCachedResult('groupsHasTransitiveDependency', cacheKey, true);
        }
        ...
        if (targetTopParent !== ctx.targetTopParent) {
            ...
            targets                 = ctx.targets = {};
            // Pushing the target also reclculates its parents ..so include them into affected targets
            targetGroup.forEach(function (task) { targets[ task.getId() ] = true; });
        }
        ...
        // If source is a summary task we will also iterate its children since moving the source
        // will cause moving its children which might have effect on the target 
        if (source.childNodes && source.childNodes.length) {
            source.cascade(function (task) {
                sourceGroup.push(task);
            });
        }

        // for each source group task we check if any of its succeeding task is in "targets" hash
        sourceGroup.forEach(function (task) {
